...
void roundRobinSinultaneouslySendRecv(int rank, int numProcs) {
	const int MAX_RAND = 20;
	long int mineRand = 0, prevRand = 0;
	int nextRank = (rank + 1) % numProcs;
	int prevRank = rank == 0 ? numProcs - 1 : rank - 1;
	...
	MPI_Sendrecv(static_cast<void*>(&mineRand), 1, MPI_LONG, nextRank, 1,
		static_cast<void*>(&prevRand), 1, MPI_LONG, prevRank, 1,
...
int main(int argc, char** argv) {
	...
	roundRobinSinultaneouslySendRecv(rank, numProcs);
